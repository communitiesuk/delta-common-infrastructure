...
resource "aws_lambda_function" "test_lambda" {
  ...
  environment {
    variables = {
      group_name = aws_cloudwatch_log_group.emails_sent,
      event_type = ?????,
      log_level = "INFO",
    }
  }
  timeout = 60
  handler = index(aws_lambda_function)
#  line above seems questionable (yaml line was Handler: index.lambda_handler)
  runtime = "python3.8"
  memory_size = 128
  code = ?????
  depends_on = [
