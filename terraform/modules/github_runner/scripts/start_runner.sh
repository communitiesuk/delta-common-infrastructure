...
sudo --preserve-env=RUNNER_ALLOW_RUNASROOT -u "$user_name" -- ./config.sh --unattended --name "delta-common-infra-$environment" --work "_work" --url https://github.com/communitiesuk/delta-common-infrastructure --token ${github_token} --labels $environment
