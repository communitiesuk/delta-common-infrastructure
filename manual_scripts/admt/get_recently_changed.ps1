...
Get-AdUser -SearchBase "CN=datamart,CN=Users,$sourceBase" -Filter {WhenChanged -gt $date -and MemberOf -eq $deltaGroup.DistinguishedName} -Server $server| Select-Object @{n='SourceName';e={$_.SamAccountName}} | Export-Csv -Path .\recent-users-includefile.csv -NoTypeInformation
